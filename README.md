...
2. **전파인증 / 안전인증** 중 조회할 항목 체크
   - 전파인증 **소스 선택**: `로컬DB(즉시)` 또는 `RRA 실시간` (일괄 조회 전용 드롭다운)
   - 안전인증은 AuthKey + 로컬 프록시 필요
3. **일괄 조회** → 모델별 결과 표(인증번호·인증상태·건수·상세링크) 표시
4. **CSV 다운로드** — 매칭된 **모든 건**을 한 줄씩, 전체 필드로 저장
   (입력모델·인증종류·인증번호·인증상태·모델명·기자재명칭/제품명·제조국·인증일자·
   업체/제조사·수입사·비고; 엑셀 호환 UTF-8 BOM)
